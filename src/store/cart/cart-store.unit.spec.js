...
describe('CartStore', () => {
  ...
  it('should remove a product from the store', () => {
    const [product, otherProduct] = server.createList('product', 2);
    ...
    expect(result.current.state.products).toHaveLength(1);
    expect(result.current.state.products[0]).toEqual(otherProduct);
  });

  it('should not change products in the cart if provided product is not in the array', () => {
    const [product, otherProduct, moreOtherProduct] = server.createList('product', 3);
